...
from ecoguard.analyzers.air_pollution.settlement_ranking import rank_settlement_candidates
from ecoguard.analyzers.air_pollution.transport_corridor import (
    TransportCorridorScreeningResult,
    apply_transport_corridor,
)
from ecoguard.analyzers.air_pollution.transport_geometry import downwind_to_direction_deg
from ecoguard.analyzers.air_pollution.transport_spatial_output import (
    PollutionTransportSpatialOutput,
    prepare_transport_spatial_output,
)
from ecoguard.analyzers.air_pollution.transport_time import (
    apply_transport_time_estimate,
    estimate_corridor_settlement_transport_time,
)
from ecoguard.analyzers.air_pollution.ims_wind_evidence_service import IMSWindEvidenceService
from ecoguard.analyzers.air_pollution.ims_wind_observation_client import IMSWindObservationClient
    ...
    def predict(
        ...
        *,
        ...
        origin = (
            ...
            else AnalysisOrigin.model_validate(
                analysis_origin.model_dump()
                if isinstance(analysis_origin, AnalysisOrigin)
                else analysis_origin
            )
        )
        if (
            origin.analysis_origin_kind != "monitoring_location"
            or origin.analysis_origin_coordinates != validated.anomaly.location
        ):
            raise ValueError("transport service requires the candidate monitoring origin")
        evidence = (
