"""Grounded non-emergency Air Pollution response-planning contracts."""
...
from ecoguard.analyzers.air_pollution.event_analysis_schemas import AirPollutionEventAnalysis
from ecoguard.shared.schemas import ProtocolCitation

PlanningStatus = Literal["success", "failed", "skipped"]
PlanningReason = Literal[
    "insufficient_analysis_evidence",
...
class AirPollutionRecommendedAction(AnomalyContract):
    """Grounded advice only; never allocation, dispatch, or action confirmation."""
    ...
    supporting_analysis_evidence_ids: list[str] = Field(default_factory=list, max_length=12)
    spatial_relevance: str | None = Field(default=None, min_length=10, max_length=1000)


class AirPollutionPlanProposal(AnomalyContract):
    """Structured model proposal before deterministic grounding verification."""
    ...
    def _actions_use_declared_types(self) -> "AirPollutionPlanProposal":
        ...
        if not authorities.issubset(set(self.recommended_authority_types)):
            raise ValueError("actions use an undeclared authority type")
        if not resources.issubset(set(self.recommended_resource_types)):
            raise ValueError("actions use an undeclared resource type")
        return self


class VerifiedPollutionProtocolReference(AnomalyContract):
...
class AirPollutionResponsePlan(AnomalyContract):
    """Non-emergency recommendations or an explicit fail-closed result."""
    ...
    analysis_evidence_references: list[str] = Field(default_factory=list)
    ...
    def _status_is_coherent(self) -> "AirPollutionResponsePlan":
        if self.status == "success":
            ...
            or self.actions
            or self.recommended_authority_types
            or self.recommended_resource_types
...
class AirPollutionPlanningResult(AnomalyContract):
    """Preserve the complete Analyzer report alongside the planning outcome."""
