...
# The first eight are the fire-weather basics every consumer already reads.
# The last three describe how dry the fuel and the ground under it are, which
# the basics only imply: two days at the same temperature and humidity leave
# very different fuel behind depending on how much water was already in the
# soil and how fast the air pulled it out. All eleven come from one call, so
# the extra three cost nothing but the wider response.
#
# Rows written before these were added simply do not carry them. Nothing
# re-fetches a closed hour, so the history deepens forward from here rather
# than filling in backwards.
HOURLY_VARIABLES = (
    "temperature_2m", "relative_humidity_2m", "precipitation", "rain",
    "wind_speed_10m", "wind_direction_10m", "wind_gusts_10m", "weather_code",
    "soil_moisture_0_to_7cm", "vapour_pressure_deficit", "et0_fao_evapotranspiration",
)
