...
from ecoguard.detectors.air_pollution.five_minute_baseline_import import build_import_plan
...
def test_adapter_creates_only_draft_five_minute_identity(monkeypatch):
    ...
    monkeypatch.setattr(
        "ecoguard.detectors.air_pollution.five_minute_baseline_import.validate_artifacts",
    ...
    monkeypatch.setattr(
        "ecoguard.detectors.air_pollution.five_minute_baseline_import.build_completed_hour_plan",
...
def test_cli_keeps_double_write_guard_and_lazy_database_import():
    source = open("ecoguard/scripts/import_air_pollution_five_minute_baselines.py", encoding="utf-8").read()
