...
from ecoguard.detectors.air_pollution.baseline_import import (
    HISTORICAL_AGGREGATION_POLICY, HISTORICAL_BASELINE_FAMILY,
    HISTORICAL_QUALITY_POLICY, build_import_plan,
)
from ecoguard.detectors.air_pollution.baseline_import_validation import SCHEMA, TIME_RULE
...
def test_dry_run_cli_database_import_is_lexically_guarded():
    source = open("ecoguard/scripts/import_air_pollution_baselines.py", encoding="utf-8").read()
...
def test_dry_run_cli_emits_json_only_on_stdout(monkeypatch, capsys):
    import ecoguard.scripts.import_air_pollution_baselines as cli
