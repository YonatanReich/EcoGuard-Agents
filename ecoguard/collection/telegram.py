...
from agents.telegram_fire_listener import CHANNELS, get_session_path, load_credentials
    ...
    async def _collect(self) -> list[dict[str, Any]]:
        ...
        try:
            if not await client.is_user_authorized():
                ...
                # job cannot do. Run agents/telegram_fire_listener.py once to
                # create the session; this collector then reuses it forever.
                raise RuntimeError(
                    "Telegram session is not authorised. Run "
                    "`python -m agents.telegram_fire_listener` once to log in."
                )
