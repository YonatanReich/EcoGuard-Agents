...
def test_feature_build_is_as_of_and_preserves_actual_fallback_run(monkeypatch):
    ...
    monkeypatch.setattr("research.pilots.evaluate_historical_forecast_deterioration_pilot._weather_checkpoint",lambda label:Checkpoint(weather))
    predictor=Predictor()
...
def test_schema_and_no_network_or_production_coupling():
    ...
    source=inspect.getsource(__import__("research.pilots.evaluate_historical_forecast_deterioration_pilot",fromlist=["x"]))
