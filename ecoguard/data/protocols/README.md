...
`ProtocolRetriever(hazard="fire")` reads `ecoguard/data/protocols/fire`. One retriever instance
...
5. Add an entry to that hazard's `manifest.json`, including the SHA-256:
   ```
   python -c "import hashlib,pathlib; p=pathlib.Path('ecoguard/data/protocols/fire/<file>.md'); print(hashlib.sha256(p.read_bytes()).hexdigest())"
   ```
...
1. Create `ecoguard/data/protocols/<hazard>/` with at least one document and a `manifest.json`
