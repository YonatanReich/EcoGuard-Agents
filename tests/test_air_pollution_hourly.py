...
from services.air_pollution_hourly import (
...
from test_ministry_air_quality_client import FakeSession, routes_for, channel, station
...
def test_no_db_or_network_imports():
    ...
    tree = ast.parse(Path("services/air_pollution_hourly.py").read_text(encoding="utf-8"))
    imports = [n.module for n in ast.walk(tree) if isinstance(n, ast.ImportFrom)]
    assert imports == ["datetime", "urllib.parse", "services.air_quality_schemas"]
