...
from scripts.build_historical_landcover_terrain_features import LAND_COVER_FEATURES, TERRAIN_FEATURES
from scripts.train_fire_prediction_landcover_terrain_models import (
    ABLATIONS, FULL_FEATURES, assert_feature_allowlist,
)
from scripts.train_fire_prediction_environmental_models import FULL_FEATURES as CURRENT_FEATURES
from scripts.train_fire_prediction_models import FORBIDDEN_FEATURES
...
def test_saved_model_feature_order_and_temporal_selection_metadata():
    path = Path("data/generated/ml/fire_prediction_landcover_terrain_model.joblib")
    if path.exists():
...
def test_saved_metrics_select_model_from_validation_only():
    path = Path("data/generated/ml/fire_prediction_landcover_terrain_metrics.json")
    if path.exists():
