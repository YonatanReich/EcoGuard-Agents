...
import pytest
import requests
import scripts.build_historical_firms_dataset as historical_firms

from scripts.build_historical_firms_dataset import (
    CheckpointError,
    CheckpointStore,
    FirmsProviderError,
    ProductAvailability,
...
def test_progress_accounting_and_checkpoint_resume(tmp_path, monkeypatch):
    monkeypatch.setattr("scripts.build_historical_firms_dataset.time.sleep", MagicMock())
    logs = []
