...
- `agents/fire_risk_prediction_agent.py` → `training.calibrate_fire_risk_levels.apply_calibration`
- `agents/response_plan_judge_agent.py` → `evaluation.evaluation_schemas`
- `services/current_risk_feature_builder.py` → `datasets.build_historical_environmental_features`, `datasets.build_historical_fire_negative_samples`
- `services/static_feature_store.py` → `datasets.build_historical_landcover_terrain_features`
